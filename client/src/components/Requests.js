            ...
            requests.map((val, key)=>(
                <tr key={key}>
                    ...
                    <td>
                    {
                      ...
                      <>
                      <button onClick={()=>acceptReq(val['reqId'])}>accept</button>
                      <button onClick={()=>rejectReq(val['reqId'])}>reject</button>
                      </>
                      :
                      <></>
                    }
                    {
                      ...
                      <button onClick={()=>doPayment(val['owner'], val['buyer'], val['landId'].toNumber())}>pay</button>
                      :
                      <></>
                    }
                    {
                      (props.user == val['owner'] && reqStatus[val['requestStatus']]==reqStatus[2])?
                      <button onClick={()=>verifyPayment(val['reqId'])}>paymentDone</button>
                      :
                      <></>
                    }
                    </td>
